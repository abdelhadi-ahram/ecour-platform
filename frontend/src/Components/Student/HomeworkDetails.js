...
function StudentAnswer({answer, isOpen}){
	return(
	<div className="w-full bg-white rounded-xl px-4 rounded-md shadow border border-green-300">
		<div className="flex flex-col space-y-1 py-2 ">
			<div className="flex items-center justify-between">
				<p className="text-gray-600 font-semibold">My answer:</p>
				{isOpen && <button className="px-3 py-1 bg-blue-50 rounded-lg font-semibold text-blue-400">Update</button>}
			</div>
...
function HomeworkDetails(){
	...
	return(
		<div className="flex flex-col space-y-3">
			<div className="w-full rounded-xl py-2 px-4 bg-white flex justify-between items-center">
				<div className="flex items-center space-x-2">
					<Link to={`/my/element/${data?.getHomeworkContent.section.element.id}`}>
						<p className="text-gray-400 hover:text-gray-700 ">{data?.getHomeworkContent.section.element.name}</p>
					</Link> 
					<span>/</span>
				...
				{ data?.getHomeworkContent.isOpen ? (
						<div className="rounded-full bg-green-50 px-3 py-2 flex items-center space-x-2 text-green-500 text-sm">
						...
						<div className="rounded-full bg-red-50 px-3 py-2 flex items-center space-x-2 text-red-500 text-sm">
			...
			<div className="w-full bg-white rounded-xl px-4 py-1 rounded shadow">
