...
function ColorPicker(){
  ...
  function Palette(){
    ...
    return (
      <div className="grid grid-cols-4 gap-x-1 gap-y-1 grid-flow-row auto-rows-max absolute top-2 right-0 w-48 overflow-auto bg-white rounded shadow z-20 p-2 border border-gray-200">
        {COLORS.map(color => {
  ...
  function ColorGradient({color}){
    ...
    return(
      <div className="absolute top-2 right-0 w-48 bg-white rounded shadow z-20 p-1 border border-gray-200">
        ...
        <div className="grid grid-cols-4 gap-x-1 gap-y-1 grid-flow-row auto-rows-max w-48 bg-white p-2">
