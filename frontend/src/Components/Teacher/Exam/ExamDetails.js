...
function Question({question, index}){
	return (
		<div className="border border-gray-300 dark:border-zinc-600 rounded-md py-2 px-3">
			<div className="flex space-x-2">
				<div className="py-2">
					<div className="rounded-full bg-zinc-200 text-gray-500 dark:text-gray-400 dark:bg-zinc-700 w-6 h-6 flex items-center justify-center">{index}</div>
				</div>
				<div className="w-full flex flex-col space-y-1">
					...
					<div className="flex flex-col space-y-1">
					{
						question.choices.map((choice, index) => { return (
							<div key={index} className={`w-full p-2 flex rounded border justify-between ${choice.isCorrect ? "text-blue-400 border-blue-400 bg-blue-50 dark:border-zinc-600 dark:bg-zinc-700" : "text-gray-600 dark:text-gray-300 border-gray-400 dark:border-zinc-700 dark:bg-zinc-800"}`}>
								<p>{choice.content}</p>
								<span className={`${choice.isCorrect ? "text-blue-50 dark:text-gray-300" : "text-gray-600 dark:text-gray-400"}`}>
		...
		<div className="flex flex-col space-y-3">
			<div className="w-full rounded-xl py-2 px-4 bg-white dark:bg-zinc-800 flex justify-between items-center">
				...
				<div className="flex justify-end">
					<Link to={`/my/home/edit-exam/${examId}`}><button className="update-btn">Update</button></Link>
				</div>
			</div>
				...
				<div className="w-full flex flex-col space-y-2 py-3">
					<div className="flex flex-col space-y-1">
						<div className="flex justify-between items-center">
							<p className="text-gray-500 dark:text-gray-400 font-semibold">Description</p>
							<div className="flex items-center space-x-2 px-2">
								<svg className="w-5 h-5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
								<p className="text-gray-700 dark:text-gray-300 text-sm">{moment.utc(data?.getExamById.duration*1000).format("hh:mm")}</p>
							</div>
						</div>
						<div className="px-2">
							{
					...
					<div className="flex flex-col space-y-1">
						<p className="text-gray-500 dark:text-gray-400 font-semibold">Starts at</p>
						<div className="px-2 text-gray-700 dark:text-gray-300">
							{
					...
					<div className="flex flex-col space-y-1">
						<p className="text-gray-500 dark:text-gray-400 font-semibold">Questions</p>
