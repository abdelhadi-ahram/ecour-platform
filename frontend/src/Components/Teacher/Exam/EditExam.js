		...
		{/* left Sidebar for users */}
			<div className="w-full lg:w-1/4 rounded-lg p-1 lg:grow-0 lg:overflow-y-auto">
					<div className="flex flex-col space-y-3">
						...
						<div className="w-full rounded-md bg-zinc-800 py-2 px-3 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-3 gap-2">
								{
									questions.map((item, index) => {
										const warning = (!item.type.id || parseInt(item.mark) === 0 || (item.type.type != "Plain text" && item.choices.length == 0) )
										return <div onClick={() => setSelected(index)} key={index} className={`w-full cursor-pointer bg-zinc-700 py-[4px] rounded-lg flex items-center justify-center text-gray-200 text-md border ${selected == index ? "border-blue-400 text-blue-400":`${warning ? "border-yellow-500 text-yellow-500" : "border-zinc-600 hover:border-zinc-500"}`}`}>Q-{index+1}</div>
									})
								}
								<div onClick={addQuestion} className="w-full bg-zinc-800 border border-zinc-600 hover:border-zinc-500 p-[4px] rounded-lg text-gray-200 text-md flex justify-center items-center">
