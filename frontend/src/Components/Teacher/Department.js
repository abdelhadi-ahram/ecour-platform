      ...
      <div className="py-1 sticky top-0 bg-gray-100">
    		<div className="rounded-xl bg-white flex overflow-hidden shadow p-1">
    			{DEPARTMENT.map((item,index) => {
    				...
    				return(
    					<div onClick={() => {setSelected(index)}} key={index} className={`py-2 w-32 flex justify-center items-center rounded-lg cursor-pointer font-semibold ${isSelected ? "bg-blue-50 text-blue-400" : "bg-white text-gray-400 hover:text-gray-500"}`}>{item.name}</div>
    				)
    			})}
    		</div>
      </div>
